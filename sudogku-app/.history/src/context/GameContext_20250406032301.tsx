...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const [displayMode, setDisplayMode] = useState<DisplayMode>('dogs')
  ...
  const checkPuzzleCompletion = (currentBoard: (number | null)[][]) => {
    // First check if all cells are filled
    if (!SudokuSolver.isSolved(currentBoard)) return false

    // Then check if the solution matches
  ...
  const setCellValue = (row: number, col: number, value: number | null) => {
    ...
    setBoard((prev) => {
      ...
      // Check if puzzle is complete with the new board state
      if (checkPuzzleCompletion(newBoard)) {
        setIsComplete(true)
        setShowVictory(true)
        updateStats()
      }
  ...
  const checkSolution = () => {
    return checkPuzzleCompletion(board)
  }
